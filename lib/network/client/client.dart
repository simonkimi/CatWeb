...
class NetClient {
  ...
  Future<ListParserResult> getList({
    ...
    final content = req.data!;
    final parserStr = jsonEncode(parser.toJson());

    final rsp = await callFFiParser(
      content,
      ParserModelType.list,
      parserStr,
    );
  ...
  Future<DetailParserResult> getDetail({
    ...
    final parseRsp = await callFFiParser(
      rsp.data!,
      ParserModelType.detail,
      jsonEncode(parser.toJson()),
    );

    final result = DetailParserResult.fromJson(parseRsp);
    _checkSuccessFlag(result.isSuccess, result.failMessage);
    siteService.currentSite?.updateGlobalEnv(result.envs);
    return result;
  }
  ...
  Future<ImageReaderResult> getReadImage({
    ...
    final parserRsq = await callFFiParser(
      rsp.data!,
      ParserModelType.imageReader,
      jsonEncode(parser.toJson()),
    );

    final result = ImageReaderResult.fromJson(parserRsq);
    _checkSuccessFlag(result.isSuccess, result.failMessage);
    siteService.currentSite?.updateGlobalEnv(result.envs);
    return result;
  }
  ...
  Future<AutoCompleteResult> getAutoComplete({
    ...
    final parseRsp = await callFFiParser(
      rsp.data!,
      ParserModelType.autoComplete,
      jsonEncode(parser.toJson()),
    );

    final result = AutoCompleteResult.fromJson(parseRsp);
    _checkSuccessFlag(result.isSuccess, result.failMessage);
    siteService.currentSite?.updateGlobalEnv(result.envs);
    return result;
  }
...
Future<Map<String, dynamic>> callFFiParser(
  String content,
  ParserModelType parserType,
  String parser,
) async {
  final rsp = await parseHtmlAsync(content, parserType.type, parser);
  final json = jsonDecode(rsp);
